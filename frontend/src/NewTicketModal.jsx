...
export default function NewTicketModal({ isOpen, onClose, onSuccess, projectId }) {
  ...
  const gridGroup = "grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-5";
  ...
  const modalFooter = "px-5 sm:px-6 md:px-8 py-4 sm:py-5 border-t border-slate-200/60 bg-slate-50/30 flex flex-col-reverse sm:flex-row justify-end items-center gap-3 sm:gap-4 rounded-b-2xl sm:rounded-b-3xl";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${baseUrl}/api/projects/${projectId}/tickets`, {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        setError(data.error || data.err || 'Failed to create ticket');
      }
    } catch (err) {
        ...
        <form onSubmit={handleSubmit}>
          <div className={formSection}>
            ...
            <div className={inputGroup}>
              ...
              <input id="title" type="text" placeholder="e.g. Fix navbar logic" value={title} onChange={(e) => setTitle(e.target.value)} className={inputStyle} required />
            </div>
